...
export class BuffetController {
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      //Create consecutive
      const consecutive = await consecutiveController.get_next_consecutive_code('Buffets');
      if (!consecutive)
        return res.status(404).json({ mensaje: "No se pudo crear el consecutivo" });
      req.body.code = consecutive;

      // Measure unit exists?
      const measure_unit_code: string = req.body.measure_unit_code;
      const measure_unit = await MeasureUnitModel.findOne({
        code: measure_unit_code,
      });
      if (!measure_unit)
        return res
          .status(404)
          .json({ mensaje: "No se encontro la unidad de medida" });
  ...
  async update(req: Request, res: Response, next: NextFunction) {
    ...
    if (!req.body)
      return res.status(404).json({ mensaje: "No se recibieron datos" });
    if (id !== req.body._id)
      return res.status(404).json({ mensaje: "El id no coincide" });
    try {
      // Measure unit exists?
      const measure_unit_code: string = req.body.measure_unit_code;
      const measure_unit = await MeasureUnitModel.findOne({
      ...
      if (!measure_unit)
        return res
          .status(404)
          .json({ mensaje: "No se encontro la unidad de medida" });
      const entityUpdate = await BuffetModel.findByIdAndUpdate(id, req.body, {
        new: true,
      });
      if (!entityUpdate)
        return res.status(404).json({ mensaje: "No se encontro la entidad" });
