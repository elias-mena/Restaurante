...
export class ProveedoresService {
  ...
  route = 'http://localhost:3000/proveedor/';
  ...
  async put(id: string, item: Proveedores) {
    const data = await axios
  ...
  add(item: Proveedores) {
    return Promise.resolve(this.post(item));
  }

  modify(id: string, item: Proveedores) {
